...
# Using a larger learning rate for the physical parameter (c) often prevents 
# it from getting stuck while the network weights are still adapting.
...
for epoch in range(1, epochs + 1):
    ...
    # --- Physics Domain Pass (Higher-order AD) ---
    x_p = model(t_physics)
    ...
    # Parametrized residual calculation: d2x/dt2 + c*dx/dt + 400*x = 0
    ...
    if epoch % plot_every == 0 or epoch == 1:
        ...
        ax.set_title(
            f"PINN vs Analytical (Epoch: {epoch} | Est. c: {model.c.item():.3f})"
        )
...
# 7. Generate Parameter Convergence Plot
print("\nGenerating parameter estimation history plot...")
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(epoch_history, c_history, "b-", linewidth=2, label="Estimated Damping Coefficient (c)")
ax.axhline(y=true_c, color="r", linestyle="--", linewidth=2, label=f"True Value (c = {true_c})")
ax.set_title("Damping Coefficient ($c$) Convergence History")
ax.set_xlabel("Epoch")
...
print(f"Convergence graph saved to: {parameter_plot_path}")
